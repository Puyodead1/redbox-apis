...
import { readUsers, saveUsers, updateUser, createCPN, getUserByProfileNumber } from "./utils";
...
*/
const TIER_MULTIPLIER: { [key: string]: number } = { // points per $1.00 spent based on tier (for used disc purchases)
...
export function estimateAccrual(user: User, data: any = {}, excludeRentals = false) {
...
export function calculateTier(purchases: number): Tier {
  if (purchases >= Number(process.env.TIER_LEGEND_PURCHASES || 50)) return "Legend"; // 50+ purchases minimum (default)
  if (purchases >= Number(process.env.TIER_SUPERSTAR_PURCHASES || 20)) return "Superstar"; // 20+ purchases minimum (default)
  if (purchases >= Number(process.env.TIER_STAR_PURCHASES || 10)) return "Star"; // 10+ purchases minimum (default)
  if (purchases >= Number(process.env.TIER_MEMBER_PURCHASES || 0)) return "Member"; // No minimum requirement (default)
...
export async function updateRewards(barcode: string, transaction: any) {
  ...
  const item = transaction.items.Rental.find((item: any) => item.returnedDate && item.Barcode == barcode.toString()); // find the barcode in the transaction
  if(!item) return;
  ...
  if (user) { // if there's a rewards account
      ...
      let NIGHTS_COUNT = Math.max(1, Math.ceil((returnDate.getTime() - rentalDate.getTime()) / (1000 * 60 * 60 * 24)));
...
export async function createAccount(data: any): Promise<{ success: boolean; reason?: string; data?: any }> {
  ...
  const pin = data.Pin ? encService.decrypt(data.Pin, EncryptionType.LOCAL) : null;
