...
export const post = [
    ...
    async (req: Request, res: Response) => {
        ...
        if (isEmailAuth || isPinAuth) {
            ...
            const getUser = isEmailAuth ? await loyalty.getUserByEmail(identifier) : await loyalty.getUserByPhoneNumber(identifier);
            const encService = new EncryptionService();
            let authentication = body.IsEncrypted ? encService.decrypt(credential, EncryptionType.CUSTOMER) : credential;
